...
Okay so that's a great idea, but like, what does that _mean_? Currently,
...
In atproto, _users_ create _records_ that are cryptographically signed to
demonstrate authorship. Records have a schema called a _Lexicon_.

Records are stored in _repositories_. Repositories run as a _service_, exposing
...
network, and doing things with them. These services all called _App Views_,
...
run. To fix this, there's an additional kind of service, called a _relay_, that
...
propogates posts from a smaller subset of users too. They don't _have_ to show
...
out into its own kind of service in atproto, called a _feed generator_. A classic
...
A _Labeler_ is a service that applies _labels_ to content or accounts. As a user,
...
application level, they actually go _further_ than in other systems. If someone
...
PDS and want to ignore posts that come from a certain one, but that is _your_
...
or _[DID][did]_. My DID looks like this: `did:plc:3danwc67lo7obz2fmdg6jxcr`.
Feel free to follow me! Lol, of course that's not the interface that you'll see
most of the time. Identity also involves a _handle_, which is a domain name.
...
<BlueskyPost uri="at://did:plc:3danwc67lo7obz2fmdg6jxcr/app.bsky.feed.post/3km6y2x6cxk2f" />
